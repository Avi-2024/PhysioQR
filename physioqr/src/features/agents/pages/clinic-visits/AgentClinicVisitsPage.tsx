...
import { CalendarClock, CheckCircle2, MapPin, Plus, Search, Stethoscope } from 'lucide-react';
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-neutral-500">Record the doctor meeting and schedule the next follow-up only when needed.</p>
        </div>
      ...
      <section className="rounded-xl border border-neutral-200 bg-white shadow-sm">
        ...
        <div className="p-4 sm:p-5">
          ...
          {!visitsQuery.isLoading && filtered.length > 0 && (
            <div className="space-y-3">{filtered.map((visit) => <VisitCard key={text(visit._id || visit.id)} visit={visit} />)}</div>
          )}
        </div>
      </section>
...
function VisitCard({ visit }: { visit: ApiRecord }) {
  ...
  return (
    <article className="rounded-xl border border-neutral-200 p-4 transition-colors hover:border-primary-200">
      ...
      {(text(visit.followUpDate) || text(visit.nextAction)) && (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {text(visit.followUpDate) && (
            <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3 text-xs text-amber-800">
              <strong>Follow-up:</strong> {dateText(visit.followUpDate)}{text(visit.followUpType) ? ` • ${labelize(visit.followUpType)}` : ''}
            </div>
          )}
...
function VisitForm({ doctors, saving, error, onSubmit }: { doctors: ApiRecord[]; saving: boolean; error: unknown; onSubmit: (payload: ApiRecord) => void }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (followUpRequired && !followUpDate) {
      setFormError('Follow-up date is required when follow-up is enabled.');
      return;
    }
    ...
    onSubmit({
      ...
      outcome,
      followUpStatus: followUpRequired ? 'scheduled' : 'not_required',
      followUpDate: followUpRequired ? followUpDate : undefined,
      followUpType: followUpRequired ? followUpType : undefined,
      nextAction: followUpRequired ? form.get('nextAction') || undefined : undefined,
    });
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="rounded-lg border border-primary-100 bg-primary-50/50 p-4 text-sm text-primary-900">
        Record the visit. If Follow-up required is Yes, saving the visit automatically places it in the Follow-ups workspace.
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Linked doctor">
          <select value={doctorId} onChange={(event) => setDoctorId(event.target.value)} className={inputClass}>
            <option value="">Unlinked / new doctor</option>
            {doctors.map((doctor) => <option key={text(doctor._id || doctor.id)} value={text(doctor._id || doctor.id)}>{text(doctor.fullName)} — {text(doctor.clinicName, 'No clinic')}</option>)}
          </select>
        </Field>
        ...
        <Field label="Visit outcome">
          <select value={outcome} onChange={(event) => setOutcome(event.target.value)} className={inputClass}>
            {OUTCOMES.map(([value, itemLabel]) => <option key={value} value={value}>{itemLabel}</option>)}
          </select>
        </Field>
        <Field label="Doctor interest">
          <select name="doctorInterestLevel" className={inputClass}>
            <option value="">Not captured</option>
            {INTEREST.map(([value, itemLabel]) => <option key={value} value={value}>{itemLabel}</option>)}
          </select>
        </Field>
        <Field label="Discussion details" wide><textarea name="discussionDetails" className={`${inputClass} min-h-24`} placeholder="What was discussed during the visit?" /></Field>

        <Field label="Follow-up required">
          <select value={followUpRequired ? 'yes' : 'no'} onChange={(event) => setFollowUpRequired(event.target.value === 'yes')} className={inputClass}>
      ...
      {followUpRequired && (
        <div className="rounded-xl border border-amber-200 bg-amber-50/40 p-4">
          <p className="text-sm font-bold text-neutral-900">Schedule follow-up</p>
          <p className="mt-1 text-xs text-neutral-500">This will automatically appear in the Agent Follow-ups workspace after the visit is saved.</p>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Field label="Follow-up date"><input type="date" required value={followUpDate} onChange={(event) => setFollowUpDate(event.target.value)} className={inputClass} /></Field>
            <Field label="Follow-up type">
              <select value={followUpType} onChange={(event) => setFollowUpType(event.target.value)} className={inputClass}>
                {FOLLOW_UP_TYPES.map(([value, itemLabel]) => <option key={value} value={value}>{itemLabel}</option>)}
              </select>
            </Field>
            <Field label="Next action" wide><input name="nextAction" className={inputClass} placeholder="e.g. Call doctor, revisit clinic, send WhatsApp details" /></Field>
          </div>
        </div>
      )}
...
function Metric({ label, value, icon: Icon }: { label: string; value: number; icon: React.ElementType }) {
  return <div className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm"><span className="inline-flex rounded-lg bg-primary-50 p-2 text-primary-700"><Icon className="h-5 w-5" /></span><p className="mt-3 text-2xl font-bold text-neutral-900">{value}</p><p className="text-sm text-neutral-500">{label}</p></div>;
}

function Field({ label, children, wide }: { label: string; children: React.ReactNode; wide?: boolean }) {
  return <label className={wide ? 'block sm:col-span-2' : 'block'}><span className="text-sm font-semibold text-neutral-700">{label}</span><div className="mt-2">{children}</div></label>;
}

function Pill({ value, muted }: { value: string; muted?: boolean }) {
...
function errorMessage(error: unknown) { const response = asRecord(asRecord(error).response); const data = asRecord(response.data); return text(data.message || asRecord(error).message, 'Unable to save clinic visit.'); }
