...
import { Clock3, Pencil, Plus, QrCode, Search, ShieldCheck, Stethoscope, XCircle } from 'lucide-react';
...
export default function AgentDoctorsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesSearch = !q || [row.fullName, row.doctorId, row.clinicName, row.city, row.specialization, row.mobile]
  ...
  const columns: DataTableColumn<DoctorRow>[] = [
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-neutral-500">{row.doctorId}</p>
        </div>
      ),
    },
    {
      ...
      render: (row) => (
        <div>
          <p className="text-sm font-medium text-neutral-800">{row.clinicName}</p>
    ...
    },
    {
      ...
      render: (row) => <span className="text-sm text-neutral-700">{row.specialization}</span>,
    },
    {
      key: 'mobile',
      header: 'Contact',
      render: (row) => <span className="text-sm text-neutral-700">{row.mobile}</span>,
    },
    {
      key: 'status',
      header: 'Status',
    ...
    },
    {
      key: 'qrCodeActive',
      header: 'QR',
      render: (row) => row.qrCodeActive ? (
        <button
          type="button"
          onClick={(event) => {
            event.stopPropagation();
            navigate(`/agent/doctors/${row.id}#doctor-qr`);
          }}
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700 hover:bg-amber-100"
        >
          <QrCode className="h-3.5 w-3.5" />
          Complete setup
        </button>
      ) : (
        <span className="text-xs font-semibold text-neutral-500">Not active</span>
      ),
    },
    {
      key: 'createdAt',
      header: 'Added',
      render: (row) => <span className="text-sm text-neutral-600">{dateText(row.createdAt)}</span>,
    },
    {
      key: 'actions',
      header: 'Actions',
  ...
  if (query.isError) {
    return (
      <ErrorState
        ...
        message="Your assigned doctor list is temporarily unavailable."
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary-600">Doctor onboarding</p>
          <h1 className="mt-1 text-2xl font-bold text-neutral-900 sm:text-3xl">My Doctors</h1>
          <p className="mt-1 text-sm text-neutral-500">Doctors registered by you are approved automatically and shown here immediately.</p>
        </div>
        <button
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Kpi icon={Clock3} label="Needs attention" value={needsReview} />
      ...
      <section className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full max-w-xl">
            ...
            <input
              ...
              placeholder="Search doctor, clinic, city, specialization or mobile"
              className="w-full rounded-lg border border-neutral-300 py-2.5 pl-9 pr-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100"
            />
          </div>
          <div className="flex flex-wrap gap-2">
            {['all', 'approved', 'under_review', 'documents_required', 'rejected', 'suspended', 'inactive'].map((item) => (
              <button
                key={item}
                type="button"
                onClick={() => setStatus(item)}
                className={`rounded-full px-3 py-1.5 text-xs font-semibold capitalize ${status === item ? 'bg-primary-600 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}
              >
        ...
        <div className="mt-5">
          <DataTable
            ...
            onRowClick={(row) => navigate(`/agent/doctors/${row.id}`)}
          />
        </div>
      </section>

      <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-xs text-neutral-600">
        Agent access is operational only. Banking, payout, detailed fee-share and confidential patient medical information are not displayed.
      </div>
    </div>
  );
}
...
function StatusPill({ value }: { value: string }) {
  ...
  return (
    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold capitalize ${positive ? 'bg-emerald-50 text-emerald-700' : warning ? 'bg-amber-50 text-amber-700' : negative ? 'bg-rose-50 text-rose-700' : 'bg-neutral-100 text-neutral-700'}`}>
      {value.replace(/_/g, ' ')}
    </span>
  );
}
...
function mapDoctor(record: ApiRecord): DoctorRow {
  return {
    ...
    qrCodeActive: Boolean(record.qrCodeActive),
