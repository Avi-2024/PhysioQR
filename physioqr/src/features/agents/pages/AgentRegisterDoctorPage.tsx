...
type ProgramOption = { _id: string; programCode?: string; name: string; painCategory?: { name?: string } | null };
type CreatedDoctor = { _id: string; doctorId?: string; fullName?: string; mobile?: string; temporaryPassword?: string; status?: string };
...
export default function AgentRegisterDoctorPage() {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { revenueModel: 'split', preferredProgram: '', requestedFeeShareType: 'percentage' },
  });
  const commissionType = watch('requestedFeeShareType');
  const mutation = useMutation({
    mutationFn: async (data: FormData) => apiClient.post('/doctors', clean(data)),
    onSuccess: (response) => setCreatedDoctor(response.data as CreatedDoctor),
  });
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500">Add the essential profile and commercial setup. The doctor will be approved automatically.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Preferred Rehab Programme">
            <select {...register('preferredProgram')} className={inputClass} disabled={programsQuery.isLoading}>
              ...
              {programs.map((program) => <option key={program._id} value={program._id}>{program.name}{program.painCategory?.name ? ` — ${program.painCategory.name}` : ''}</option>)}
            </select>
          </Field>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-neutral-500">These terms become the doctor's initial active setup. Admin can change them later.</p>
        </div>

        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <Field label="Payment Model" required error={errors.revenueModel?.message}>
            <select {...register('revenueModel')} className={inputClass}><option value="split">Revenue Share (Split)</option><option value="platform_fee">Platform Fee</option></select>
          </Field>
          <Field label="Patient Price (₹)" required error={errors.requestedPatientFee?.message}><input {...register('requestedPatientFee')} type="number" min="1" max="100000" step="1" className={inputClass} placeholder="e.g. 999" /></Field>
          <Field label="Doctor Commission Type" required error={errors.requestedFeeShareType?.message}>
            <select {...register('requestedFeeShareType')} className={inputClass}><option value="percentage">Percentage (%)</option><option value="fixed">Fixed Amount (₹)</option></select>
          </Field>
          {commissionType === 'percentage' ? (
            <Field label="Doctor Commission (%)" required error={errors.requestedFeeSharePercentage?.message}><input {...register('requestedFeeSharePercentage')} type="number" min="0" max="100" step="0.01" className={inputClass} placeholder="e.g. 60" /></Field>
          ) : (
            <Field label="Doctor Commission (₹)" required error={errors.requestedFixedFeeShareAmount?.message}><input {...register('requestedFixedFeeShareAmount')} type="number" min="0" max="100000" step="1" className={inputClass} placeholder="e.g. 500" /></Field>
          )}
        </div>

        <p className="mt-5 text-xs leading-5 text-neutral-500">Preferred rehab programme is only a doctor onboarding preference. The patient's actual rehab programme is still selected from the patient's assessment and pain category.</p>
        {programsQuery.isError && <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs font-medium text-amber-800">Rehab programmes could not load. You can still register the doctor and choose it later.</div>}
