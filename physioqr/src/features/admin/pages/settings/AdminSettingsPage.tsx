...
type FinanceSettings = { currency: 'INR'; refundsEnabled: boolean; withdrawalsEnabled: boolean };
...
const sectionMeta = {
  ...
  finance: { title: 'Finance controls', description: 'High-level availability controls for money workflows.', icon: CircleDollarSign },
    ...
    {(Object.keys(sectionMeta) as Section[]).map((section) => {
      ...
      return <section key={section} className="rounded-2xl border border-neutral-200 bg-white">
        ...
        <div className="grid gap-4 p-5 md:grid-cols-2">
          ...
          {section === 'finance' && <><label className="text-sm font-medium text-neutral-700">Currency<input value={draft.finance.currency} disabled className="mt-2 w-full rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2.5 font-normal text-neutral-500" /></label><div /><Toggle label="Refund workflows enabled" description="Administrative availability flag; refund processing still follows the dedicated refund domain." checked={draft.finance.refundsEnabled} onChange={(v) => setDraft({ ...draft, finance: { ...draft.finance, refundsEnabled: v } })} /><Toggle label="Withdrawal workflows enabled" description="Administrative availability flag; wallet balances must never be edited directly." checked={draft.finance.withdrawalsEnabled} onChange={(v) => setDraft({ ...draft, finance: { ...draft.finance, withdrawalsEnabled: v } })} /></>}
