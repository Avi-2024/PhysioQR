...
export default function AdminAssessmentDetailPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess:() => {
      setShowEdit(false); setFormError('');
  ...
  const statusMutation = useMutation({
    ...
    onSuccess:() => {
      setShowStatus(false); setFormError('');
      queryClient.invalidateQueries({ queryKey:['admin-assessment-question', assessmentId] });
      queryClient.invalidateQueries({ queryKey:['admin-assessment-questions'] });
      queryClient.invalidateQueries({ queryKey:['assessment-questions'] });
    },
  ...
  return <div className="mx-auto w-full max-w-[1500px] space-y-5">
    <header className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6"><div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div className="flex min-w-0 items-start gap-4"><button type="button" onClick={() => navigate('/admin/assessments')} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-neutral-200 text-neutral-600"><ArrowLeft className="h-5 w-5"/></button><div className={cn('flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl', question.isRedFlag ? 'bg-rose-50 text-rose-700' : 'bg-primary-50 text-primary-700')}>{question.isRedFlag ? <AlertTriangle className="h-7 w-7"/> : <ClipboardList className="h-7 w-7"/>}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="max-w-4xl text-xl font-bold tracking-tight text-neutral-950 sm:text-2xl">{question.questionText}</h1><span className={cn('rounded-full px-2.5 py-1 text-xs font-semibold', question.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-neutral-100 text-neutral-600')}>{question.isActive ? 'Active' : 'Inactive'}</span>{question.isRedFlag && <span className="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700">Red flag</span>}</div><p className="mt-2 text-sm text-neutral-600">{typeLabel(question.questionType)} · Common assessment question</p><p className="mt-1 text-xs text-neutral-500">Updated {dateText(question.updatedAt)}</p></div></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => query.refetch()} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-neutral-300 px-3.5 text-sm font-semibold text-neutral-700"><RefreshCw className={cn('h-4 w-4', query.isFetching && 'animate-spin')}/>Refresh</button><button type="button" onClick={() => setShowEdit(true)} className="inline-flex min-h-10 items-center rounded-lg bg-primary-600 px-4 text-sm font-semibold text-white">Edit configuration</button></div></div></header>

    <section className="rounded-xl border border-primary-200 bg-primary-50 p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-primary-700"/><div><div className="text-sm font-bold text-primary-950">One common assessment</div><p className="mt-1 text-sm leading-6 text-primary-800">This question is not tied to a pain category. Pain category is chosen by the patient inside the assessment and later used for program mapping.</p></div></div></section>

    {question.isRedFlag && <section className="rounded-xl border border-rose-200 bg-rose-50 p-4"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 text-rose-700"/><div><div className="text-sm font-bold text-rose-950">Safety-sensitive question</div><p className="mt-1 text-sm leading-6 text-rose-800">Matching answers send the assessment to Risk Reviews. Changes here affect future submissions.</p></div></div></section>}

    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><main className="space-y-5"><Card title="Question content"><Item label="English" value={question.questionText}/><Item label="Hindi" value={question.questionTextHindi || '—'}/><Item label="Type" value={typeLabel(question.questionType)}/><Item label="Assessment" value="Common for all pain categories"/><Item label="Display order" value={String(question.displayOrder ?? 0)}/><Item label="Status" value={question.isActive ? 'Active' : 'Inactive'}/></Card><Card title="Answer configuration"><Item label="Options" value={question.options?.length ? question.options.map((option) => option.label || option.value).filter(Boolean).join(', ') : 'Not required / not configured'}/><Item label="Option count" value={String(question.options?.length || 0)}/></Card><Card title="Red-flag rule"><Item label="Enabled" value={question.isRedFlag ? 'Yes' : 'No'}/><Item label="Operator" value={typeLabel(question.redFlagOperator)}/><Item label="Trigger values" value={valuesText(question.redFlagAnswerValues)}/><Item label="Range" value={question.redFlagMinValue !== undefined || question.redFlagMaxValue !== undefined ? `${question.redFlagMinValue ?? '—'} to ${question.redFlagMaxValue ?? '—'}` : '—'}/><Item label="Safety message" value={question.redFlagSafetyMessage || '—'}/></Card><Card title="Conditional visibility"><Item label="Conditional" value={conditional ? 'Yes' : 'No'}/><Item label="Depends on" value={parent?.questionText || 'Always shown'}/><Item label="Expected answer" value={question.showIfAnswer || (question.conditionalLogic?.value !== undefined ? String(question.conditionalLogic.value) : '—')}/><Item label="Operator" value={typeLabel(question.conditionalLogic?.operator || (conditional ? 'equals' : undefined))}/></Card></main>

      <aside className="space-y-4 xl:sticky xl:top-5"><section className="overflow-hidden rounded-xl border border-neutral-200 bg-white"><div className="border-b border-neutral-100 bg-neutral-50/60 px-4 py-4"><h2 className="text-sm font-bold text-neutral-950">Configuration actions</h2><p className="mt-1 text-xs text-neutral-500">Use status changes instead of hard deletion so historical assessments stay explainable.</p></div><div className="space-y-2 p-4"><button type="button" onClick={() => setShowEdit(true)} className="w-full rounded-xl border border-neutral-200 px-4 py-3 text-left text-sm font-semibold text-neutral-700">Edit question and rules</button><button type="button" onClick={() => { setFormError(''); setShowStatus(true); }} className={cn('w-full rounded-xl border px-4 py-3 text-left text-sm font-semibold', question.isActive ? 'border-rose-200 text-rose-700' : 'border-emerald-200 text-emerald-700')}>{question.isActive ? 'Deactivate question' : 'Reactivate question'}</button><button type="button" onClick={() => navigate('/admin/risk-reviews')} className="flex w-full items-center gap-2 rounded-xl border border-neutral-200 px-4 py-3 text-left text-sm font-semibold text-neutral-700"><AlertTriangle className="h-4 w-4"/>Open Risk Reviews</button></div></section><section className="rounded-xl border border-neutral-200 bg-white p-4"><div className="flex items-center gap-2 text-sm font-bold text-neutral-950"><GitBranch className="h-4 w-4 text-primary-700"/>Rule summary</div><div className="mt-4 space-y-3 text-sm"><div className="flex justify-between gap-3"><span className="text-neutral-500">Assessment</span><span className="font-semibold text-neutral-900">Common</span></div><div className="flex justify-between gap-3"><span className="text-neutral-500">Red flag</span><span className="font-semibold text-neutral-900">{question.isRedFlag ? 'Yes' : 'No'}</span></div><div className="flex justify-between gap-3"><span className="text-neutral-500">Conditional</span><span className="font-semibold text-neutral-900">{conditional ? 'Yes' : 'No'}</span></div></div></section></aside>
    </div>

    <Modal isOpen={showEdit} onClose={() => { setShowEdit(false); setFormError(''); }} title="Edit common assessment question" size="xl"><form onSubmit={handleUpdate} className="space-y-6 pt-5">{formError && <div className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{formError}</div>}<div className="grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2"><span className="text-xs font-semibold text-neutral-600">Question text *</span><textarea name="questionText" required rows={3} defaultValue={question.questionText} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm"/></label><label className="sm:col-span-2"><span className="text-xs font-semibold text-neutral-600">Hindi text</span><textarea name="questionTextHindi" rows={2} defaultValue={question.questionTextHindi || ''} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm"/></label><label><span className="text-xs font-semibold text-neutral-600">Type</span><select name="questionType" defaultValue={question.questionType} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm">{['single_choice','multiple_choice','yes_no','pain_scale','number','text','date','image'].map((type) => <option key={type} value={type}>{typeLabel(type)}</option>)}</select></label><label><span className="text-xs font-semibold text-neutral-600">Display order</span><input name="displayOrder" type="number" min="0" defaultValue={question.displayOrder} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"/></label><label className="sm:col-span-2"><span className="text-xs font-semibold text-neutral-600">Options — one per line, optional value after |</span><textarea name="options" rows={4} defaultValue={question.options?.map((option) => `${option.label || option.value || ''}|${option.value || option.label || ''}`).join('\n') || ''} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm"/></label><label className="flex items-center gap-3 rounded-lg border border-neutral-200 px-3 py-3 sm:col-span-2"><input name="isRedFlag" type="checkbox" defaultChecked={question.isRedFlag}/><span className="text-sm font-semibold">Red-flag safety rule</span></label><label><span className="text-xs font-semibold text-neutral-600">Red-flag operator</span><select name="redFlagOperator" defaultValue={question.redFlagOperator || 'any_answer'} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm">{['any_answer','equals','not_equals','includes','gte','lte','between'].map((operator) => <option key={operator} value={operator}>{typeLabel(operator)}</option>)}</select></label><label><span className="text-xs font-semibold text-neutral-600">Trigger values, comma separated</span><input name="redFlagAnswerValues" defaultValue={valuesText(question.redFlagAnswerValues) === '—' ? '' : valuesText(question.redFlagAnswerValues)} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"/></label><label><span className="text-xs font-semibold text-neutral-600">Min value</span><input name="redFlagMinValue" type="number" defaultValue={question.redFlagMinValue ?? ''} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"/></label><label><span className="text-xs font-semibold text-neutral-600">Max value</span><input name="redFlagMaxValue" type="number" defaultValue={question.redFlagMaxValue ?? ''} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"/></label><label className="sm:col-span-2"><span className="text-xs font-semibold text-neutral-600">Safety message</span><textarea name="redFlagSafetyMessage" rows={3} defaultValue={question.redFlagSafetyMessage || ''} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm"/></label><label><span className="text-xs font-semibold text-neutral-600">Show only after question</span><select name="showIfQuestion" defaultValue={parent?._id || ''} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"><option value="">Always shown</option>{parentQuery.data?.items.filter((item) => item._id !== question._id).map((item) => <option key={item._id} value={item._id}>{item.questionText}</option>)}</select></label><label><span className="text-xs font-semibold text-neutral-600">Expected answer</span><input name="showIfAnswer" defaultValue={question.showIfAnswer || ''} className="mt-1 min-h-11 w-full rounded-lg border border-neutral-300 px-3 text-sm"/></label></div><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowEdit(false)} className="rounded-lg border border-neutral-300 px-4 py-2.5 text-sm font-semibold">Cancel</button><button disabled={updateMutation.isPending} className="rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{updateMutation.isPending ? 'Saving…' : 'Save changes'}</button></div></form></Modal>

    <Modal isOpen={showStatus} onClose={() => setShowStatus(false)} title={question.isActive ? 'Deactivate question' : 'Reactivate question'} size="md"><StatusForm active={!question.isActive} pending={statusMutation.isPending} error={formError} onCancel={() => setShowStatus(false)} onSubmit={(reason) => statusMutation.mutate({ active:!question.isActive, reason })}/></Modal>
  </div>;
}

function StatusForm({ active, pending, error, onCancel, onSubmit }:{ active:boolean; pending:boolean; error:string; onCancel:()=>void; onSubmit:(reason:string)=>void }) {
  const [reason, setReason] = useState('');
  return <div className="space-y-4 pt-4"><p className="text-sm text-neutral-600">{active ? 'Reactivate this question in the common assessment.' : 'Remove this question from future common assessments. Historical answers remain unchanged.'}</p><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={4} placeholder="Reason (required)" className="w-full rounded-lg border border-neutral-300 p-3 text-sm"/>{error && <p className="text-sm text-rose-700">{error}</p>}<div className="flex justify-end gap-2"><button type="button" onClick={onCancel} className="rounded-lg border border-neutral-300 px-4 py-2 text-sm font-semibold">Cancel</button><button type="button" disabled={!reason.trim() || pending} onClick={() => onSubmit(reason.trim())} className="rounded-lg bg-primary-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{pending ? 'Saving…' : active ? 'Reactivate' : 'Deactivate'}</button></div></div>;
}
