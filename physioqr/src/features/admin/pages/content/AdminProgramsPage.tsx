  ...
  return <div className="min-w-0 space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm leading-6 text-neutral-500">Manage rehabilitation content mapped to body regions. Programmes do not carry a price; patient fees are controlled only by the referring Doctor or Admin.</p>
      </div>
    ...
    <Modal isOpen={showCreate} onClose={() => { setShowCreate(false); setFormError(''); }} title="Create rehabilitation program" size="lg">
      <form onSubmit={handleCreate} className="space-y-5 pt-5">
        <p className="text-sm text-neutral-600">Every program must map to an active pain category. Programme creation is clinical-content setup only; patient fees are configured separately by Doctor or Admin.</p>
