...
export default function AdminDashboardPage() {
  ...
  const KpiCard = ({ title, value, icon: Icon, color, sub, onClick }: { title: string; value: string | number; icon: React.ElementType; color: string; sub?: string; onClick?: () => void }) => (
    <button onClick={onClick} className={cn('card p-5 text-left w-full hover:shadow-card-hover transition-shadow', onClick && 'cursor-pointer')}>
      {isLoading ? (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => refetch()} className="flex items-center gap-2 px-4 py-2.5 border border-neutral-300 text-neutral-700 text-sm font-medium rounded-lg hover:bg-neutral-50 transition-colors">
            <RefreshCw className="w-4 h-4" /> Refresh
          </button>
          <button onClick={() => navigate('/admin/doctors/new')} className="flex items-center gap-2 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg transition-colors">
        ...
        <div className="card p-5">
          ...
          <div className="space-y-3">
            {stats.pendingDoctors.map((doc) => (
              <div key={doc.id} className="flex items-center justify-between py-3 border-b border-neutral-100 last:border-0">
                <div>
        ...
        <div className="card p-5">
          ...
          <div className="space-y-3">
            {stats.recentPatients.map((pat) => (
              <div key={pat.id} className="flex items-center justify-between py-3 border-b border-neutral-100 last:border-0">
                <div>
