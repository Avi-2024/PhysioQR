...
export default function AdminRefundDetailPage() {
  ...
  const query = useQuery<any>({
    ...
    queryFn: () => apiClient.get(`/admin/refunds/${refundId}`).then((response) => response.data),
  });
  ...
  const canProcess = ['requested', 'approved', 'failed'].includes(r.status);
  const actionLabel = r.status === 'failed' ? 'Retry Razorpay' : 'Send to Razorpay';
  ...
  return <div className="space-y-6">
    <header>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary-700">{duplicate ? 'Duplicate charge refund' : 'Gateway refund review'}</p><h1 className="mt-2 text-2xl font-bold text-neutral-950">{r.gatewayRefundId || `Refund ${r._id.slice(-8)}`}</h1><p className="mt-1 text-sm text-neutral-500">{r.patient?.fullName || 'Patient'} · {label(r.refundType)}</p></div>
        <div className="flex flex-wrap gap-2">{canProcess && <button disabled={processRefund.isPending} onClick={() => processRefund.mutate()} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-neutral-950 px-4 text-sm font-semibold text-white disabled:opacity-50"><Send className="h-4 w-4" />{processRefund.isPending ? 'Submitting…' : actionLabel}</button>}<button onClick={() => query.refetch()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border px-4 text-sm font-semibold"><RefreshCw className={cn('h-4 w-4', query.isFetching && 'animate-spin')} />Refresh</button></div>
      </div>
    </header>
    ...
    <div className={cn('rounded-xl border p-4 text-sm', r.status === 'completed' ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : r.status === 'failed' ? 'border-red-200 bg-red-50 text-red-900' : 'border-amber-200 bg-amber-50 text-amber-900')}>
      <div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><strong>{r.status === 'completed' ? 'Refund confirmed by gateway.' : r.status === 'processing' ? 'Razorpay is processing this refund.' : r.status === 'failed' ? 'Gateway refund failed.' : 'Refund is waiting for gateway submission.'}</strong><p className="mt-1 leading-6">{duplicate ? 'The primary payment, patient program and original doctor fee share remain unchanged.' : 'Payment status, fee-share reversal and program cancellation are applied only after confirmed gateway completion.'}</p></div></div>
    </div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="rounded-xl border bg-white p-5"><h2 className="font-bold">Refund & gateway</h2><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2">{[
        ['Gateway refund ID', r.gatewayRefundId], ['Gateway status', label(r.gatewayStatus)], ['Gateway amount', r.gatewayAmount == null ? '—' : money(r.gatewayAmount)], ['Last gateway event', label(r.lastGatewayEvent)], ['Status', label(r.status)], ['Type', label(r.refundType)], ['Reason', r.reason], ['Failure reason', r.rejectionReason], ['Requested by', r.requestedBy?.email || r.requestedBy?.mobile], ['Processed by', r.processedBy?.email || r.processedBy?.mobile], ['Last gateway event at', r.lastGatewayEventAt ? new Date(r.lastGatewayEventAt).toLocaleString() : '—'], ['Processed at', r.processedAt ? new Date(r.processedAt).toLocaleString() : '—'],
    ...
    <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-600">Razorpay submission and financial reversal are separate stages. A refund is considered financially complete only after a matching gateway confirmation is reconciled.</div>
  </div>;
}
