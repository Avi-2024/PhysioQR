  ...
  return <div className="mx-auto w-full max-w-[1500px] space-y-5">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_350px]"><main className="space-y-5">
      ...
      <aside className="space-y-4 xl:sticky xl:top-5">
        <section className="overflow-hidden rounded-xl border border-neutral-200 bg-white"><div className="border-b border-neutral-100 bg-neutral-50/60 px-4 py-4"><h2 className="text-sm font-bold text-neutral-950">Account actions</h2><p className="mt-1 text-xs text-neutral-500">Status changes and deletion are audited.</p></div><div className="space-y-2 p-4">{patient.status !== 'active' && <button type="button" onClick={() => openStatus('active')} className="flex w-full items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-left text-sm font-semibold text-emerald-800"><CircleCheckBig className="h-4 w-4" />Activate patient</button>}{patient.status !== 'inactive' && <button type="button" onClick={() => openStatus('inactive')} className="flex w-full items-center gap-3 rounded-xl border border-neutral-200 px-4 py-3 text-left text-sm font-semibold text-neutral-700 hover:bg-neutral-50"><Ban className="h-4 w-4" />Set inactive</button>}{patient.status !== 'blocked' && <button type="button" onClick={() => openStatus('blocked')} className="flex w-full items-center gap-3 rounded-xl border border-rose-200 px-4 py-3 text-left text-sm font-semibold text-rose-700 hover:bg-rose-50"><ShieldAlert className="h-4 w-4" />Block patient</button>}{canDelete ? <button type="button" onClick={openDelete} className="flex w-full items-center gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-left text-sm font-semibold text-rose-700"><Trash2 className="h-4 w-4" />Delete patient permanently</button> : <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-xs leading-5 text-amber-800">Permanent deletion is disabled because this patient has successful payment or locked referral history. Use Inactive or Blocked instead.</div>}</div></section>
    ...
    {deleteOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onMouseDown={() => !deleteMutation.isPending && setDeleteOpen(false)}><div className="w-full max-w-lg rounded-2xl border border-rose-200 bg-white shadow-xl" onMouseDown={(e) => e.stopPropagation()}><div className="border-b border-rose-100 px-5 py-4"><div className="flex items-center gap-2 text-rose-700"><Trash2 className="h-5 w-5"/><h2 className="text-lg font-bold">Delete patient permanently</h2></div><p className="mt-2 text-sm leading-6 text-neutral-600">This removes the unpaid patient account and linked onboarding data. This cannot be undone.</p></div><div className="space-y-4 p-5"><label className="block text-sm font-semibold text-neutral-800">Reason<textarea value={deleteReason} onChange={(e) => setDeleteReason(e.target.value)} rows={3} placeholder="Why is this patient being deleted?" className="mt-2 w-full rounded-xl border border-neutral-300 px-3 py-2.5 text-sm outline-none focus:border-rose-500" /></label><label className="block text-sm font-semibold text-neutral-800">Type mobile number <span className="font-bold text-rose-700">{patient.mobile}</span> to confirm<input value={deleteConfirmation} onChange={(e) => setDeleteConfirmation(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-neutral-300 px-3 text-sm outline-none focus:border-rose-500" /></label>{actionError && <p className="text-sm font-medium text-rose-600">{actionError}</p>}<div className="flex justify-end gap-2 border-t border-neutral-100 pt-4"><button type="button" disabled={deleteMutation.isPending} onClick={() => setDeleteOpen(false)} className="rounded-lg border border-neutral-300 px-4 py-2.5 text-sm font-semibold text-neutral-700">Cancel</button><button type="button" disabled={deleteMutation.isPending || !deleteReason.trim() || deleteConfirmation.trim() !== patient.mobile.trim()} onClick={() => deleteMutation.mutate({ confirmation: deleteConfirmation.trim(), reason: deleteReason.trim() })} className="rounded-lg bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">{deleteMutation.isPending ? 'Deleting…' : 'Delete permanently'}</button></div></div></div></div>}
  </div>;
}