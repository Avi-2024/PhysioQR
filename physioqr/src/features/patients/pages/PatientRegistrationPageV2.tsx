...
export default function PatientRegistrationPageV2() {
  ...
  const handleCategoryChange = (categoryId: string) => {
    setSelectedCategory(categories.find((item) => text(item._id || item.id) === categoryId) || {});
    setError('');
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-50 px-3 py-5 sm:px-4 sm:py-8">
      <div className="mx-auto max-w-3xl">
        <header className="mb-5 flex items-center justify-between gap-3 border-b border-neutral-200 bg-white px-4 py-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <img src="/PhysioQR.png" alt="PhysioQR" className="h-10 w-auto shrink-0 object-contain" />
            <div className="min-w-0">
              <h1 className="font-bold text-neutral-950">Patient Registration</h1>
              <p className="truncate text-xs text-neutral-500">{doctorCode ? `Referred by doctor ${doctorCode}` : 'Secure patient onboarding'}</p>
            </div>
          </div>
          <span className="hidden text-xs font-semibold text-primary-700 sm:inline">Step {step} of 6</span>
        </header>
          ...
          <div className="p-4 sm:p-7">
            ...
            {step === 1 && (
              <form onSubmit={basicForm.handleSubmit(handleBasicSubmit)} className="space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Gender" error={basicForm.formState.errors.gender?.message}><select {...basicForm.register('gender')} defaultValue="" className={inputClass}><option value="" disabled>Select gender</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></Field>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                <div className="space-y-3 border-y border-neutral-200 py-4 text-sm leading-6 text-neutral-600"><p><strong className="text-neutral-900">Medical disclaimer:</strong> This programme is not emergency medical care. Stop if you experience severe pain, chest discomfort, dizziness, or worsening symptoms.</p><p><strong className="text-neutral-900">Programme consent:</strong> You confirm that the information you provide is accurate and consent to programme access and reminders.</p></div>
                <label className="flex cursor-pointer items-start gap-3 text-sm text-neutral-700"><input type="checkbox" checked={consentAccepted} onChange={(event) => setConsentAccepted(event.target.checked)} className="mt-0.5 h-5 w-5 rounded text-primary-600" /><span>I accept the terms, privacy policy, medical disclaimer, and exercise programme consent.</span></label>
            ...
            {step === 4 && (
              <div className="space-y-6">
                <SectionTitle title="Health assessment" description="Choose your pain area and answer the questions below." />

                <div>
                  <label htmlFor="pain-category" className="mb-2 block text-sm font-semibold text-neutral-900">Where are you experiencing pain? <span className="text-rose-500">*</span></label>
                  <div className="relative">
                    <select id="pain-category" value={selectedCategoryId} onChange={(event) => handleCategoryChange(event.target.value)} disabled={categoriesQuery.isLoading || categoriesQuery.isError} className={cn(inputClass, 'appearance-none pr-10')}>
                      <option value="">{categoriesQuery.isLoading ? 'Loading pain areas...' : 'Select your pain area'}</option>
                      {categories.map((category) => <option key={text(category._id || category.id)} value={text(category._id || category.id)}>{text(category.name, 'Unnamed category')}</option>)}
                    </select>
                    <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" />
                  </div>
                  {categoriesQuery.isError && <p className="mt-2 text-xs font-medium text-rose-600">Pain areas could not be loaded. Please refresh and try again.</p>}
                </div>

                <div className="border-t border-neutral-200 pt-5">
                  <h3 className="text-base font-bold text-neutral-950">Assessment questions</h3>
                  <p className="mt-1 text-sm text-neutral-500">Answer based on how you feel today.</p>

                  {questionsQuery.isLoading && <p className="mt-4 text-sm text-neutral-500">Loading assessment questions...</p>}
                  {!questionsQuery.isLoading && questions.length === 0 && <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Assessment questions are not configured yet. Please contact support.</div>}

                  <div className="mt-5 divide-y divide-neutral-200">
                    {questions.map((question, index) => (
                      <QuestionInput key={text(question._id || question.id)} index={index} question={question} value={assessment[text(question._id || question.id)]} onChange={(value) => setAssessment((current) => ({ ...current, [text(question._id || question.id)]: value }))} />
                    ))}
                  </div>
                </div>
            ...
            {step === 5 && (
              <div className="space-y-5">
                {hasRedFlag ? <div className="space-y-3 border border-rose-200 bg-rose-50 p-6 text-center"><AlertTriangle className="mx-auto h-11 w-11 text-rose-600" /><h2 className="text-lg font-bold text-rose-900">Clinical safety review required</h2><p className="text-sm text-rose-700">Your assessment contains answers that need clinical review before programme activation.</p><button onClick={() => navigate('/')} className="mt-3 rounded-lg bg-rose-600 px-6 py-3 text-sm font-semibold text-white">Return home</button></div> : <><SectionTitle title="Your assigned programme" description="Matched using your selected pain area and common assessment." /><div className="border-y border-neutral-200 py-5">{quoteQuery.isLoading ? <p className="text-sm text-primary-700">Loading programme and price...</p> : <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="font-bold text-neutral-950">{text(quoteProgram.name, 'Programme not configured')}</h3><p className="mt-1 text-sm text-neutral-600">{text(quoteProgram.description, 'Doctor-guided rehabilitation programme')}</p><div className="mt-3 flex flex-wrap gap-2"><Badge>{text(quoteProgram.durationDays, '-')} days</Badge><Badge>{labelize(quoteProgram.difficultyLevel || 'beginner')}</Badge><Badge>{text(quoteDoctor.fullName, 'Doctor')}</Badge></div></div><div className="shrink-0 sm:text-right"><p className="text-2xl font-bold text-primary-700">{formatCurrency(payable)}</p><p className="text-xs text-neutral-500">Payable amount</p></div></div>}</div><NavButtons onBack={prevStep} onNext={nextStep} nextDisabled={!quoteProgram.id || quoteQuery.isLoading} nextLabel="Proceed to Payment" /></>}
              </div>
            )}

            {step === 6 && (
              <div className="space-y-5"><SectionTitle title="Complete payment" description="Secure payment activates your rehabilitation programme." /><div className="space-y-3 border-y border-neutral-200 py-4"><AmountRow label="Programme Fee" value={Number(pricing.originalAmount || payable)} /><AmountRow label="Discount" value={-Number(pricing.discountAmount || 0)} /><div className="h-px bg-neutral-200" /><AmountRow label="Total Payable" value={payable} strong /></div><NavButtons onBack={prevStep} onNext={() => paymentMutation.mutate()} nextDisabled={paymentMutation.isPending || !payable} nextLabel={paymentMutation.isPending ? 'Processing...' : `Pay ${formatCurrency(payable)} & Activate`} /></div>
            )}
          </div>
        </main>
      </div>
...
function SectionTitle({ title, description }: { title: string; description: string }) {
  return <div><h2 className="text-xl font-bold tracking-tight text-neutral-950">{title}</h2><p className="mt-1 text-sm leading-6 text-neutral-500">{description}</p></div>;
}
...
function QuestionInput({ question, value, onChange, index }:{ question:ApiRecord; value:unknown; onChange:(value:unknown)=>void; index:number }) {
  ...
  return (
    <div className="py-5 first:pt-0 last:pb-0">
      <label className="block text-sm font-semibold leading-6 text-neutral-900">{index + 1}. {questionText}</label>
      <div className="mt-2.5">
        {type === 'pain_scale' ? (
          <div><div className="mb-2 flex justify-between text-xs text-neutral-500"><span>No pain</span><span>Worst pain</span></div><input type="range" min="0" max="10" step="1" value={Number(value ?? 0)} onChange={(event) => onChange(Number(event.target.value))} className="w-full accent-primary-600" /><p className="mt-1 text-sm font-semibold text-primary-700">Pain level: {Number(value ?? 0)}/10</p></div>
        ) : type === 'number' ? (
          <input type="number" value={text(value)} onChange={(event) => onChange(event.target.value === '' ? '' : Number(event.target.value))} className={inputClass} placeholder="Enter your answer" />
        ) : type === 'date' ? (
          <input type="date" value={text(value)} onChange={(event) => onChange(event.target.value)} className={inputClass} />
        ) : type === 'yes_no' ? (
          <div className="flex gap-3">{['yes', 'no'].map((option) => <button key={option} type="button" onClick={() => onChange(option)} className={cn('min-h-11 min-w-24 rounded-lg border px-5 text-sm font-semibold capitalize', value === option ? 'border-primary-600 bg-primary-600 text-white' : 'border-neutral-300 bg-white text-neutral-700')}>{option}</button>)}</div>
        ) : type === 'single_choice' ? (
          <div className="relative"><select value={text(value)} onChange={(event) => onChange(event.target.value)} className={cn(inputClass, 'appearance-none pr-10')}><option value="">Select an answer</option>{options.map((option, optionIndex) => { const optionValue = text(option.value || option.label); return <option key={`${optionValue}-${optionIndex}`} value={optionValue}>{text(option.label || option.value)}</option>; })}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" /></div>
        ) : type === 'multiple_choice' ? (
          <div className="space-y-2">{options.map((option, optionIndex) => { const optionValue = text(option.value || option.label); const current = Array.isArray(value) ? value.map(String) : []; return <label key={`${optionValue}-${optionIndex}`} className="flex cursor-pointer items-center gap-2 text-sm text-neutral-700"><input type="checkbox" checked={current.includes(optionValue)} onChange={(event) => onChange(event.target.checked ? [...current, optionValue] : current.filter((item) => item !== optionValue))} className="h-4 w-4 rounded text-primary-600" />{text(option.label || option.value)}</label>; })}</div>
        ) : (
          <textarea value={text(value)} onChange={(event) => onChange(event.target.value)} className={cn(inputClass, 'min-h-24 resize-y')} placeholder="Type your answer here" />
        )}
      </div>
    </div>
  );
