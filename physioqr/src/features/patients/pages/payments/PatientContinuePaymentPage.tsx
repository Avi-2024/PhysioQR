...
export default function PatientContinuePaymentPage() {
  ...
  if (nextAction === 'dashboard' || Boolean(status.programActivated)) {
    return <Message title="Your programme is already active" body="Your payment is complete and your rehabilitation programme is ready." action="Open dashboard" onAction={() => navigate('/patient/dashboard')} />;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl py-6 sm:py-10">
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-6 sm:p-8">
        ...
        <h1 className="mt-5 text-2xl font-bold text-neutral-950">Complete programme payment</h1>
        <p className="mt-2 text-sm leading-6 text-neutral-600">Your clinical review is cleared. Confirm your assigned programme and complete the secure payment to activate rehabilitation access.</p>

        {quoteQuery.isLoading ? <div className="mt-6"><Loading text="Loading programme and price..." compact /></div> : quoteQuery.isError ? <div className="mt-6 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-medium text-rose-700">{requestError(quoteQuery.error)}</div> : (
          <>
