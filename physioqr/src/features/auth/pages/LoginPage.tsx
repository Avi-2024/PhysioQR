...
export default function LoginPage() {
  ...
  const initialMode: LoginMode =
    preferredRole === "patient" ? "patient" : "credentials";
  ...
  const credentialLogin = async (data: CredentialForm) => {
    ...
    try {
      ...
      if (!["admin", "agent", "doctor"].includes(role)) {
        setSubmitError(
          role === "patient"
            ? "Patients sign in with mobile OTP."
            : "This account cannot use password sign in.",
        );
        return;
      }
      ...
      <div className="max-w-4xl w-full bg-white rounded-2xl shadow-modal overflow-hidden grid grid-cols-1 lg:grid-cols-2 border border-neutral-200">
        <div className="bg-gradient-to-br from-primary-900 via-primary-800 to-neutral-900 p-8 text-white flex-col justify-between sm:flex">
          <div>
            <Link
              to="/"
              className="flex items-center gap-3 mb-8 no-underline text-white"
            >
            ...
            <div className="space-y-4 my-8">
              ...
              <p className="text-sm text-primary-200 leading-relaxed">
                Patients sign in with mobile OTP. Registered professional and
                team accounts use secure credentials.
              </p>
            </div>
          </div>
          <div className="space-y-3 pt-6 border-t border-primary-700/50">
            <Info
              ...
              text="Secure role-based portal access"
            />
            <Info
              icon={<UserCheck className="w-4 h-4" />}
              text="Patient access through verified mobile OTP"
            />
            <Info
              icon={<Stethoscope className="w-4 h-4" />}
              text="Automatic routing to the correct workspace"
            />
          </div>
        </div>
        ...
        <div className="p-8 sm:p-10 flex flex-col justify-center">
          <div className="mb-6">
            ...
            <p className="text-xs text-neutral-500 mt-1">
              Use the sign-in method for your account
            </p>
          </div>
          {passwordChanged && (
          ...
          <div className="grid grid-cols-2 gap-2 p-1 bg-neutral-100 rounded-xl border border-neutral-200 mb-6">
            <button
              type="button"
              onClick={() => changeMode("patient")}
              className={`flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-lg transition-all ${mode === "patient" ? "bg-primary-600 text-white shadow-sm" : "text-neutral-600"}`}
            >
            ...
            <button
              type="button"
              onClick={() => changeMode("credentials")}
              className={`flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-lg transition-all ${mode === "credentials" ? "bg-primary-600 text-white shadow-sm" : "text-neutral-600"}`}
            >
          ...
          {mode === "patient" ? (
            <form
              onSubmit={patientForm.handleSubmit(patientLogin)}
              className="space-y-4"
            >
              <div>
              ...
              {otpSent && (
                <div>
                  ...
                  <input
                    ...
                    className={inputClass}
                  />
                  {patientForm.formState.errors.otp && (
                    <p className="mt-1 text-xs text-danger-600">
                      {patientForm.formState.errors.otp.message}
                    </p>
                  )}
                </div>
              )}
              <button
                type="submit"
                disabled={!otpSent || patientForm.formState.isSubmitting}
            ...
            <form
              onSubmit={credentialForm.handleSubmit(credentialLogin)}
              className="space-y-4"
            >
              <div>
                <label className="block text-xs font-semibold text-neutral-700 mb-1">
                ...
                {credentialForm.formState.errors.identifier && (
                  <p className="mt-1 text-xs text-danger-600">
                    {credentialForm.formState.errors.identifier.message}
                  </p>
                )}
              </div>
              <div>
              ...
              >
                <Lock className="w-4 h-4" />
                {credentialForm.formState.isSubmitting
                  ? "Signing in..."
                  : "Sign In"}
                <ArrowRight className="w-4 h-4" />
              </button>
...
const inputClass =
  "w-full min-w-0 px-3.5 py-2.5 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500";
const primaryButton =
  "w-full mt-2 py-3 px-4 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-all flex items-center justify-center gap-2 shadow-sm text-sm disabled:opacity-60";
function Info({ icon, text: value }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-3 text-xs text-primary-200">
      <span className="text-success-400">{icon}</span>
      <span>{value}</span>
    </div>
  );
}
...
function errorMessage(error: unknown) {
  ...
  return text(
    data.message || asRecord(error).message,
