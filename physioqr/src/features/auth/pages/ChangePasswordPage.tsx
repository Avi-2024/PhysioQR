...
export default function ChangePasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(anyErr.response?.data?.message || anyErr.message || 'Unable to change password.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-50 px-4 py-10 flex items-center justify-center">
      <div className="w-full max-w-md rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-2 text-sm leading-6 text-neutral-500">For security, the temporary password can only be used for your first sign-in. Set your own password before continuing.</p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          <Field label="Temporary password"><input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} required className={inputClass} /></Field>
          <Field label="New password"><input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required minLength={8} className={inputClass} placeholder="Minimum 8 characters" /></Field>
          <Field label="Confirm new password"><input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={8} className={inputClass} /></Field>
