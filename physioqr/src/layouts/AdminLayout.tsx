...
import {
  ...
  MessageSquare, FileSearch, AlertTriangle, Route
} from 'lucide-react';
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Clinical Reviews', path: '/admin/risk-reviews', icon: AlertTriangle },
    ],
  },
  {
    title: 'Content',
    items: [
      { label: 'Clinical Pathways', path: '/admin/assessment-pathways', icon: Route },
      { label: 'Body Regions', path: '/admin/pain-categories', icon: HeartPulse },
    ...
    <div className="min-h-screen bg-neutral-50 flex overflow-x-clip">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          <button type="button" aria-label="Close admin navigation" className="absolute inset-0 bg-neutral-950/50" onClick={() => setMobileOpen(false)} />
          <aside className="relative flex h-full w-[min(84vw,20rem)] flex-col bg-neutral-900 text-white shadow-2xl">
            <div className="h-16 flex items-center justify-between gap-3 px-4 border-b border-neutral-800">
              <div className="flex items-center gap-3 min-w-0"><Logo width={48} height={48} /><div className="min-w-0"></div></div>
              <button onClick={() => setMobileOpen(false)} className="p-2 rounded-lg text-neutral-300 hover:bg-neutral-800 hover:text-white" aria-label="Close menu"><X className="w-5 h-5" /></button>
            </div>
      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        <header className="min-h-16 bg-white border-b border-neutral-200 sticky top-0 z-20 px-4 sm:px-6 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 text-neutral-600 hover:bg-neutral-100 rounded-lg">{mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button>
          ...
          <div className="relative hidden min-w-[280px] max-w-md flex-1 md:block">
            ...
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search doctor, patient, payment, ticket" className="h-10 w-full rounded-lg border border-neutral-200 bg-neutral-50 pl-9 pr-3 text-sm focus:border-primary-500 focus:bg-white focus:ring-primary-500" />
            {searchQuery.trim() && <div className="absolute left-0 right-0 top-12 z-30 overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-modal"><div className="px-4 py-3 text-sm text-neutral-600">Use the search and filters inside each Admin module for live backend records.</div></div>}
          </div>

          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            <button className="p-2 text-neutral-500 hover:text-neutral-700 hover:bg-neutral-100 rounded-lg relative"><Bell className="w-5 h-5" /><span className="w-2 h-2 bg-primary-600 rounded-full absolute top-2 right-2"></span></button>
            <div className="hidden sm:block h-8 w-px bg-neutral-200"></div>
            <button onClick={handleLogout} className="hidden sm:flex items-center gap-2 text-xs font-semibold text-neutral-600 hover:text-neutral-900"><LogOut className="w-4 h-4" /> Logout</button>
          </div>
        </header>

        <main id="main-content" className="flex-1 w-full max-w-[1320px] mx-auto p-4 sm:p-6 lg:p-8"><Outlet /></main>
      </div>
    </div>
  );
}
