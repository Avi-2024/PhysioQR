...
const common = (seedKey, questionText, questionType, displayOrder, extra = {}) => ({
  seedKey, questionText, questionType, displayOrder, scopeType: 'common', isActive: true, ...extra,
});
const caseScoped = (seedKey, questionText, questionType, displayOrder, caseType, extra = {}) => ({
  seedKey, questionText, questionType, displayOrder, scopeType: 'case_type', caseType, isActive: true, ...extra,
});
const scoped = (seedKey, questionText, questionType, displayOrder, scopeType, target, extra = {}) => ({
  seedKey, questionText, questionType, displayOrder, scopeType, isActive: true,
  ...(scopeType === 'body_region' ? { bodyRegion: target } : { surgeryType: target }),
  ...extra,
});
const redFlagFields = (safetyMessage) => ({
...
async function seedBaseQuestions() {
  const globalItems = [
    ...
    common('common_doctor_restrictions', 'Has your doctor given you any exercise or movement restrictions?', 'single_choice', 100, {
      options: options([['No', 'no'], ['Yes', 'yes'], ["I'm not sure", 'not_sure']]),
    }),
    common('common_movement_description', 'How would you describe your movement right now?', 'single_choice', 120, {
      options: options([['Normal', 'normal'], ['Slightly limited', 'slightly_limited'], ['Moderately limited', 'moderately_limited'], ['Severely limited', 'severely_limited']]),
    }),
  ...
  await upsertQuestion(common('common_doctor_restrictions_detail', 'Please describe the exercise or movement restrictions your doctor gave you.', 'text', 110, {
    ...
    showIfAnswer: 'yes',
    conditionalLogic: { dependsOnQuestion: restrictions._id, operator: 'equals', value: 'yes' },
  }));
  ...
  const mskItems = [
    caseScoped('msk_problem_start', 'When did your problem start?', 'single_choice', 10, msk._id, {
      options: options([['Today', 'today'], ['1–7 days ago', '1_7_days'], ['1–4 weeks ago', '1_4_weeks'], ['1–3 months ago', '1_3_months'], ['More than 3 months ago', 'more_than_3_months'], ['Not sure', 'not_sure']]),
    }),
    caseScoped('msk_problem_onset', 'How did the problem start?', 'single_choice', 20, msk._id, {
      options: options([['Suddenly', 'suddenly'], ['Gradually', 'gradually'], ['After exercise or sport', 'after_exercise_sport'], ['After lifting', 'after_lifting'], ['After a fall or injury', 'after_fall_injury'], ['After work or another activity', 'after_work_activity'], ['No specific reason', 'no_specific_reason'], ['Other', 'other']]),
    }),
    caseScoped('msk_worst_pain_7_days', 'What was your worst pain in the last 7 days? (0–10)', 'pain_scale', 40, msk._id),
    caseScoped('msk_symptoms_worse', 'What makes your symptoms worse?', 'multiple_choice', 50, msk._id, { options: options(['Sitting', 'Standing', 'Walking', 'Bending', 'Lifting', 'Stairs', 'Exercise', 'Work', 'Sleeping', 'Other']) }),
    caseScoped('msk_symptoms_better', 'What makes your symptoms better?', 'multiple_choice', 60, msk._id, { options: options(['Rest', 'Movement', 'Exercise', 'Heat', 'Ice', 'Medication', 'Changing position', 'Other']) }),
  ];
...
async function seedSafetyQuestions() {
  ...
  const [lowerBack, msk] = await Promise.all([
    findRegion('Lower Back'),
    CaseType.findOne({ code: 'msk', isActive: true }).select('_id').lean(),
  ]);

  if (lowerBack) {
    await upsertQuestion(scoped('safety_bladder_bowel', 'Have you developed new difficulty controlling your bladder or bowel, or difficulty passing urine?', 'yes_no', 200, 'body_region', lowerBack._id, redFlagFields(urgentMessage)));
    await upsertQuestion(scoped('safety_saddle_numbness', 'Do you have new numbness or loss of feeling around your genitals, anus, or the area you would sit on a saddle?', 'yes_no', 210, 'body_region', lowerBack._id, redFlagFields(urgentMessage)));
    await upsertQuestion(scoped('safety_bilateral_leg_weakness', 'Do you have severe or worsening weakness, numbness, or tingling in both legs?', 'yes_no', 220, 'body_region', lowerBack._id, redFlagFields(urgentMessage)));
  } else {
    console.warn('Skipping lower-back neurological safety questions: Lower Back body region is not configured.');
  }

  if (msk) {
    await upsertQuestion(caseScoped('safety_serious_accident', 'Did this problem start after a serious accident or major trauma?', 'yes_no', 230, msk._id, redFlagFields(reviewMessage)));
  } else {
    console.warn('Skipping MSK trauma safety question: case type msk is not configured.');
  }

  await upsertQuestion(common('safety_fever_unwell', 'Along with this pain or problem, do you currently feel feverish, shivery, or generally very unwell?', 'yes_no', 240, redFlagFields(reviewMessage)));
}

async function seedRegionQuestions() {
  const definitions = [
    { region: 'Lower Back', questions: [
...
async function seedSurgeryQuestions() {
  ...
  if (tkr) {
    ...
    await upsertQuestion(scoped('tkr_current_symptoms', 'Which symptoms do you currently have after your knee replacement?', 'multiple_choice', 720, 'surgery_type', tkr._id, { options: options(['Pain', 'Swelling', 'Knee stiffness', 'Difficulty walking', 'Difficulty climbing stairs', 'Difficulty sitting or standing', 'Weakness', 'Other']) }));
    await upsertQuestion(scoped('tkr_walking_status', 'What is your current walking status?', 'single_choice', 730, 'surgery_type', tkr._id, { options: options(['Walking normally', 'Walking with difficulty', 'Walking with walker', 'Walking with crutches', 'Walking with cane', 'Not currently walking']) }));
  } else {
  ...
  for (const code of ligamentCodes) {
    ...
    await upsertQuestion(scoped(`${code}_current_symptoms`, 'Which symptoms do you currently have after your knee ligament surgery?', 'multiple_choice', 810, 'surgery_type', surgery._id, { options: options(['Pain', 'Swelling', 'Stiffness', 'Weakness', 'Instability', 'Difficulty walking', 'Difficulty using stairs', 'Difficulty exercising']) }));
    await upsertQuestion(scoped(`${code}_activity_level`, 'What is your current activity level?', 'single_choice', 820, 'surgery_type', surgery._id, { options: options(['Not exercising', 'Light activity', 'Moderate activity', 'Sports training', 'Competitive sport']) }));
  }
}
