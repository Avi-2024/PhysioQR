...
const gatewayAmountInRupees = (entity) => entity?.amount == null ? null : money2(Number(entity.amount) / 100);
...
const finalizeConfirmedRefund = async (refundId) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (refund.gatewayAmount != null && money2(refund.gatewayAmount) !== money2(refund.refundAmount)) {
        throw gatewayError('Gateway-confirmed refund amount does not match requested refund amount', 409);
      }

      if (refund.refundType === 'duplicate_payment') {
        refund.status = 'completed';
        refund.processedAt = refund.gatewayProcessedAt || new Date();
        await refund.save({ session });
        return;
      }

      const completedOthers = await Refund.find({
        payment: payment._id,
        status: 'completed',
        _id: { $ne: refund._id },
      }).select('refundAmount feeShareReversal').session(session).lean();
      ...
      if (feeShare && paidAmount > 0) {
        ...
        if (reversal > 0) {
          ...
          await WalletTransaction.create([{
            ...
            reason: `Fee share reversed after confirmed gateway refund. Refund ID: ${refund._id}`,
            createdBy: refund.processedBy,
            notes: alreadyWithdrawn ? 'Clawback recorded because fee share was already reserved or paid.' : undefined,
          }], { session });
...
const initiateRefund = async ({ refundId, adminUserId }) => {
  ...
  try {
    ...
    const gatewayAmount = gatewayAmountInRupees(gatewayRefund);
    await Refund.findByIdAndUpdate(refund._id, {
      $set: {
        gatewayRefundId: gatewayRefund.id,
        gatewayStatus: gatewayRefund.status,
        gatewayAmount,
        gatewayProcessedAt: status === 'completed' ? new Date() : undefined,
        lastGatewayEvent: 'refund.api_response',
        lastGatewayEventAt: new Date(),
        status,
      },
    });
    ...
    await Refund.findByIdAndUpdate(refund._id, {
      $set: {
        ...
        lastGatewayEvent: 'refund.api_failed',
        lastGatewayEventAt: new Date(),
      },
    });
    throw error;
...
const findRefundForWebhook = async (entity) => {
  const direct = await Refund.findOne({ gatewayRefundId: entity.id });
  if (direct) return direct;

  const payment = await Payment.findOne({ gatewayTransactionId: entity.payment_id }).select('_id').lean();
  if (!payment) return null;

  const amount = gatewayAmountInRupees(entity);
  const candidates = await Refund.find({
    payment: payment._id,
    status: { $in: ['processing', 'requested', 'approved'] },
    gatewayRefundId: { $in: [null, ''] },
  }).sort({ createdAt: 1 });

  const amountMatches = candidates.filter((candidate) => amount == null || money2(candidate.refundAmount) === amount);
...
const applyRazorpayRefundEvent = async (entity, eventName) => {
  ...
  const refund = await findRefundForWebhook(entity);
  ...
  const gatewayAmount = gatewayAmountInRupees(entity);

  if (gatewayAmount != null && money2(refund.refundAmount) !== gatewayAmount) {
    await Refund.findByIdAndUpdate(refund._id, {
      $set: {
        ...
        gatewayAmount,
        status: 'failed',
        rejectionReason: 'Gateway refund amount does not match the requested refund amount',
        lastGatewayEvent: eventName,
        lastGatewayEventAt: new Date(),
        processedAt: new Date(),
      },
    });
    return { matched: true, refund: await Refund.findById(refund._id), amountMismatch: true };
  }
  ...
  if (status === 'completed') update.gatewayProcessedAt = new Date();
  if (status === 'failed') {
    update.rejectionReason = entity.error_description || entity.error_reason || 'Razorpay reported refund failure';
    update.processedAt = new Date();
  }

  await Refund.findByIdAndUpdate(
    refund._id,
    { $set: update, ...(status !== 'failed' ? { $unset: { rejectionReason: 1 } } : {}) },
  );
...
module.exports = {
  initiateRefund,
  initiateDuplicateRefund: initiateRefund,
  finalizeConfirmedRefund,
  applyRazorpayRefundEvent,
};
