...
router.get('/clinics', getClinics); router.get('/clinics/:id', getClinicById); router.patch('/clinics/:id', validateSchema({ body: { clinicName:{type:'string',max:160}, clinicAddress:{type:'string',max:500}, city:{type:'string',max:100}, state:{type:'string',max:100}, postalCode:{type:'string',max:20}, clinicContact:{type:'string',max:30}, clinicEmail:{type:'string',max:160}, clinicWorkingHours:{type:'string',max:200}, googleMapsLink:{type:'string',max:1000}, clinicBranches:{type:'number',min:1,max:1000} } }), updateClinic);
router.get('/referrals', getReferrals); router.get('/referrals/:id', getReferralById);
router.get('/patients', getPatients); router.get('/patients/:id', getPatientById); router.patch('/patients/:id/status', validateSchema({ body:{ status:{type:'enum',values:['active','inactive','blocked'],required:true}, reason:{type:'string',max:500,required:true} } }), updatePatientStatus);
router.get('/assessment-questions', getAssessmentQuestions); router.get('/assessment-questions/:id', getAssessmentQuestionById); router.post('/assessment-questions', createAssessmentQuestion); router.patch('/assessment-questions/:id', updateAssessmentQuestion); router.post('/assessment-questions/:id/deactivate', deactivateAssessmentQuestion); router.post('/assessment-questions/:id/reactivate', reactivateAssessmentQuestion);
router.get('/pain-categories', getPainCategories); router.get('/pain-categories/:id', getPainCategoryById); router.post('/pain-categories', createPainCategory); router.patch('/pain-categories/:id', updatePainCategory); router.post('/pain-categories/:id/:action(deactivate|reactivate)', setPainCategoryStatus);
router.get('/programs', getPrograms); router.get('/programs/:id', getProgramById); router.post('/programs', createProgram); router.patch('/programs/:id', updateProgram); router.post('/programs/:id/:action(deactivate|reactivate)', setProgramStatus);
